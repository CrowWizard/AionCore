use gpui::{
    App, ElementId, Entity, InteractiveElement, IntoElement, ParentElement, RenderOnce, SharedString, Styled, Window,
...
use crate::components::{
    AgentItem, FileItem, InputSuggestion, InputSuggestionItem, InputSuggestionState, ModeSelectItem, ModelSelectItem,
};
...
impl ChatInputBox {
    ...
    pub fn pasted_images(mut self, images: Vec<ClipboardImage>) -> Self {
...
impl RenderOnce for ChatInputBox {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let suggestion_state_id = ElementId::NamedChild(Arc::new(self.id.clone()), "command-suggestions".into());
        ...
        let has_attachments =
            !self.pasted_images.is_empty() || !self.code_selections.is_empty() || !self.selected_files.is_empty();
            ...
            .when_some(self.title, |this, title| {
                this.child(
                    h_flex()
                        .w_full()
                        .pb_1p5()
                        .child(div().text_sm().text_color(theme.muted_foreground).child(title)),
                )
            })
            .child(
                v_flex()
                        ...
                        this.child({
                            ...
                            >| {
                                ...
                                h_flex()
                                    ...
                                    .child(div().text_size(px(11.5)).text_color(chip_text_color).child(label))
                                    .child(
                            ...
                            attachment_chips.extend(self.pasted_images.iter().enumerate().map(|(idx, image)| {
                                render_chip(
                                    "remove-image",
                                    idx,
                                    IconName::File,
                            ...
                            attachment_chips.extend(self.code_selections.iter().enumerate().map(|(idx, selection)| {
                                ...
                                let display_text = if selection.start_line == selection.end_line {
                                    ...
                                    format!("{}:{}~{}", filename, selection.start_line, selection.end_line)
                                };
                            ...
                            attachment_chips.extend(self.selected_files.into_iter().enumerate().map(
                                |(idx, file_path)| {
                    ...
                    .child(
                        // Textarea (multi-line input)
                        {
                            let mut input = InputSuggestion::new(&suggestion_state)
                                ...
                                .input(move |state| Input::new(state).appearance(false).disabled(disabled))
                                            ...
                                            h_flex()
                                                ...
                                                .child(
                                                    h_flex()
                                                        .gap_2()
                                                        .items_center()
                                                        .child(icon.size(px(16.)).text_color(if file.is_folder {
                                                            theme.accent
                                                        } else {
                                                            theme.foreground
                                                        }))
                            ...
                            .child(
                                h_flex()
                                    ...
                                    .when_some(self.agent_select.clone(), |this, agent_select| {
                                        this.child(Select::new(&agent_select).small().appearance(false).w(px(140.)))
                                    })
                                    ...
                                    .when_some(self.model_select, |this, model_select| {
                                        this.child(Select::new(&model_select).small().appearance(false))
                                    })
                                    ...
                                    .child({
                                        ...
                                        Popover::new("mcp-popover")
                                            ...
                                            .content(move |_state, _window, cx| {
                                                ...
                                                let mut content = v_flex().w(px(280.)).max_h(px(350.)).gap_2().p_3();
