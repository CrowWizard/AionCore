use gpui::{
    Anchor, App, Bounds, Context, ElementId, IntoElement, ParentElement, Pixels, Point, RenderOnce, Styled, Window,
...
impl ListDelegate for CommandSuggestionsListDelegate {
    ...
    fn render_item(&mut self, ix: IndexPath, _: &mut Window, cx: &mut Context<ListState<Self>>) -> Option<Self::Item> {
    ...
    fn set_selected_index(&mut self, ix: Option<IndexPath>, _: &mut Window, _: &mut Context<ListState<Self>>) {
...
impl RenderOnce for CommandSuggestionsPopover {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        match self.anchor_bounds {
            Some(bounds) => {
                let position = bounds.corner(Anchor::TopLeft) + Point { x: px(0.), y: -px(8.) };

                let list_state = window.use_keyed_state(self.list_id.clone(), cx, |window, cx| {
                    ListState::new(CommandSuggestionsListDelegate::new(Vec::new(), None), window, cx)
                });
                ...
                list_state.update(cx, |state, cx| {
                    ...
                    let selected = if has_commands { Some(IndexPath::default()) } else { None };
                    ...
                    anchored()
                        ...
                        .child(
                            v_flex()
                                ...
                                .child(div().w_full().h(px(140.0)).child(List::new(&list_state).size_full())),
                        ),
                )
                .with_priority(1)
                .into_any_element()
