...
use gpui::{
    Anchor, App, AppContext as _, Bounds, ClickEvent, Context, ElementId, Entity, EventEmitter, Focusable,
    InteractiveElement as _, IntoElement, KeystrokeEvent, Length, MouseButton, ParentElement as _, Pixels, RenderOnce,
    SharedString, StyleRefinement, Styled, Subscription, Window, anchored, deferred, div, prelude::FluentBuilder as _,
    px,
};
...
impl<T: InputSuggestionItem + 'static> InputSuggestionState<T> {
    ...
    pub fn with_input(input_state: Entity<InputState>, window: &mut Window, cx: &mut Context<Self>) -> Self {
    ...
    pub fn set_selected_index(&mut self, index: Option<usize>, window: &mut Window, cx: &mut Context<Self>) {
        self.selected = index;
    ...
    fn ensure_selection(&mut self) {
        if self.items.is_empty() {
            self.selected = None;
        } else if self.selected.map(|ix| ix >= self.items.len()).unwrap_or(true) {
...
impl<T: InputSuggestionItem + 'static> EventEmitter<InputSuggestionEvent<T>> for InputSuggestionState<T> {}
...
impl<T: InputSuggestionItem + 'static> RenderOnce for InputSuggestion<T> {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let (items, selected, open, bounds) = {
            ...
            (state.items.clone(), state.selected, state.open, state.input_bounds)
        };
        ...
        let list = {
            ...
            if let Some(header) = self.header.clone() {
                list = list.child(div().text_xs().text_color(cx.theme().muted_foreground).child(header));
            }
        ...
        let popover = if open {
            bounds.map(|bounds| {
                ...
                let content = match self.anchor {
                    ...
                    Anchor::BottomLeft | Anchor::BottomRight | Anchor::BottomCenter => content.bottom_1(),
