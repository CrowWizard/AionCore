...
impl UpdateChecker {
    pub fn new() -> Self {
        Self {
            check_url: "https://api.github.com/repos/sxhxliang/agent-studio/releases/latest".to_string(),
...
fn find_platform_asset(assets: &[GitHubAsset]) -> String {
    ...
    for pattern in patterns {
        if let Some(asset) = assets.iter().find(|a| {
            a.name.to_lowercase().contains(pattern) || a.browser_download_url.to_lowercase().contains(pattern)
        }) {
