...
pub fn ensure_default_config_at(path: &Path) -> Result<()> {
    ...
    let default_config =
        crate::assets::get_default_config().ok_or_else(|| anyhow::anyhow!("Failed to get embedded default config"))?;
    std::fs::write(path, default_config).with_context(|| format!("Failed to write config file: {:?}", path))?;
    Ok(())
}
