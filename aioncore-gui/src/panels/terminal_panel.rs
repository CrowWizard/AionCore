...
use gpui::{
    App, AppContext, Context, Entity, FocusHandle, Focusable, IntoElement, ParentElement, Pixels, Render, Styled,
...
use gpui_term::{
    Clear, Copy, Event, InputOrigin, Paste, SelectAll, Terminal, TerminalBuilder, TerminalConfig, TerminalContent,
    TerminalMiddleware, TerminalTheme, TerminalView, TextStyle,
};
...
impl TerminalPanel {
    ...
    /// 创建带指定工作目录的终端面板视图
    pub fn view_with_cwd(working_directory: std::path::PathBuf, window: &mut Window, cx: &mut App) -> Entity<Self> {
        cx.new(|cx| Self::new(window, cx, Some(working_directory)))
    }

    pub fn new(window: &mut Window, cx: &mut Context<Self>, working_directory: Option<std::path::PathBuf>) -> Self {
        // Load terminal configuration
        let terminal_config = TerminalConfig::load_or_create().unwrap_or_else(|_| TerminalConfig::default());
                ...
                if let Some(entity) = weak_self.upgrade() {
                    entity.update(cx, |this, cx| {
                        ...
                        let terminal_view = cx.new(|cx| {
                            let mut view = TerminalView::new_with_style(terminal.clone(), text_style, window, cx);
                ...
                h_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .when(is_ready, |el| {
                                el.child(gpui::div().w(px(8.)).h(px(8.)).rounded(px(4.)).bg(theme.success))
                            })
                            ...
                            .when(is_failed, |el| {
                                el.child(gpui::div().text_xs().text_color(theme.muted_foreground).child("Failed"))
                            }),
                    )
