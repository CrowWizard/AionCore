use gpui::{
    App, AppContext as _, Context, Entity, FocusHandle, Focusable, IntoElement, ParentElement as _, Render,
...
impl SettingsPanel {
    pub(super) fn confirm_footer(
        ...
        DialogFooter::new()
            ...
            .child(DialogAction::new().child(Button::new("ok").label(ok_text).with_variant(ok_variant)))
    }
    ...
    fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let mut panel = panel;
        panel.refresh_core_data(window, cx);
        panel
    }

    /// Handle agent configuration events
    fn on_agent_config_event(&mut self, event: &crate::core::event_bus::AgentConfigEvent, cx: &mut Context<Self>) {
