...
use gpui::{
    Anchor, AnyElement, App, AppContext, Context, Entity, FocusHandle, InteractiveElement as _, IntoElement,
    MouseButton, ParentElement as _, Render, SharedString, Styled as _, Subscription, Window, actions, div,
...
use crate::{AppState, SelectFont, SelectRadius, SelectScrollbarShow, app_menus, core::aioncore::ConnectionStatus};
            ...
            TitleBar::new()
                ...
                .child(
                    div()
                        .flex()
                        .items_center()
                        .when(!cfg!(target_os = "macos"), |this| this.child(self.app_menu_bar.clone())),
                )
                .child(
                    div()
                        .flex()
                        ...
                        .child(connection_status_label(cx))
...
fn connection_status_label(cx: &App) -> impl IntoElement {
    let (label, color) = match AppState::global(cx).core_connection_status() {
        ConnectionStatus::Connected => ("Connected", cx.theme().success),
        ConnectionStatus::Connecting => ("Connecting", cx.theme().warning),
        ConnectionStatus::Disconnected => ("Disconnected", cx.theme().danger),
    };

    div().flex().items_center().gap_1().text_color(color).child(label)
}
