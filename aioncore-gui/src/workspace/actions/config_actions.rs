...
use crate::{
    AppState,
    app::actions::{
        AddAgent, ChangeConfigPath, ReloadAgentConfig, RemoveAgent, RestartAgent, SetUploadDir, UpdateAgent,
    },
};
...
pub fn restart_agent(action: &RestartAgent, cx: &mut App) {
    ...
    let _ = cx
        .spawn(async move |_cx| match agent_config_service.restart_agent(&name).await {
            Ok(()) => {
...
pub fn reload_agent_config(_action: &ReloadAgentConfig, cx: &mut App) {
    ...
    let _ = cx
        .spawn(async move |_cx| match agent_config_service.reload_from_file().await {
            Ok(()) => {
