...
impl DockWorkspace {
    ...
    pub(in crate::workspace) fn show_conversation_panel(
        ...
        if let Some(session_id) = session_id.as_deref().filter(|id| !id.is_empty()) {
            ...
            self.dock_area.update(cx, |dock_area, cx| {
                ...
                dock_area.add_panel(Arc::new(conversation_panel), DockPlacement::Center, None, window, cx);
            });
            return;
        }
    ...
    fn find_first_tab_panel(item: &DockItem, cx: &App) -> Option<(Entity<TabPanel>, Arc<dyn PanelView>)> {
        match item {
            DockItem::Tabs { view, .. } => view
                .read(cx)
                .active_panel(cx)
                .map(|active_panel| (view.clone(), active_panel)),
            DockItem::Split { items, .. } => items.iter().find_map(|item| Self::find_first_tab_panel(item, cx)),
            DockItem::Tiles { .. } | DockItem::Panel { .. } => None,
        }
    }
    ...
    fn activate_existing_session_panel(
        ...
        log::debug!("Searching existing session panel: session_id={}", session_id);
        let items = self.dock_area.read(cx).center().clone();
        let found = Self::activate_session_in_item(&items, session_id, window, cx);
        if found {
            log::debug!("Activated existing session panel: session_id={}", session_id);
        } else {
    ...
    fn activate_session_in_item(item: &DockItem, session_id: &str, window: &mut Window, cx: &mut App) -> bool {
        match item {
            DockItem::Tabs { view, .. } => {
    ...
    fn activate_existing_session_manager_panel(&mut self, window: &mut Window, cx: &mut Context<Self>) -> bool {
    ...
    fn activate_panel_by_klass(item: &DockItem, klass: &str, window: &mut Window, cx: &mut App) -> bool {
        match item {
            DockItem::Tabs { view, .. } => {
                let tab_state = view.read(cx).dump(cx);
                let active_ix = tab_state.info.active_index().unwrap_or(0);
    ...
    pub(in crate::workspace) fn on_action_open(&mut self, _: &crate::Open, _: &mut Window, cx: &mut Context<Self>) {
