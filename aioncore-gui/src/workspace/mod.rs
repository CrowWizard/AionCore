...
impl DockWorkspace {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let title_bar = cx.new(|cx| {
            AppTitleBar::new("AionCore GUI", window, cx)
            // .child({
            //     move |_, cx| {
    ...
    fn reset_default_layout(dock_area: WeakEntity<DockArea>, window: &mut Window, cx: &mut App) {
        ...
        let bottom_panels = DockItem::split_with_sizes(
            Axis::Vertical,
            vec![DockItem::tabs(
                vec![Arc::new(DockPanelContainer::panel::<TerminalPanel>(window, cx))],
                &dock_area,
                window,
                cx,
            )],
        ...
        cx.spawn(async move |cx| {
            ...
            window
                .update(cx, |_, window, cx| {
                    ...
                    window.set_window_title("AionCore GUI");
                    cx.on_release(|_, cx| {
                        // exit app
                        cx.quit();
                    })
...
pub fn open_new(
    ...
    let task: Task<std::result::Result<WindowHandle<Root>, anyhow::Error>> = DockWorkspace::new_local(cx);
    cx.spawn(async move |cx| {
