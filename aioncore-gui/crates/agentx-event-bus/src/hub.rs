...
use agentx_types::{
    AgentConfigEvent, CodeSelectionEvent, Config, PermissionRequestEvent, SessionStatus, SessionUpdateEvent,
...
impl EventHub {
    ...
    pub fn subscribe_session_updates_for_session<F>(&self, session_id: String, callback: F) -> SubscriptionId
    where
        F: Fn(&SessionUpdateEvent) + Send + Sync + 'static,
    {
        self.subscribe_with_filter(
    ...
    pub fn subscribe_session_updates_for_agent<F>(&self, agent_name: String, callback: F) -> SubscriptionId
    where
        F: Fn(&SessionUpdateEvent) + Send + Sync + 'static,
    {
        self.subscribe_with_filter(
    ...
    pub fn subscribe_permission_requests_for_session<F>(&self, session_id: String, callback: F) -> SubscriptionId
    where
        F: Fn(&PermissionRequestEvent) + Send + Sync + 'static,
    {
        self.subscribe_with_filter(
    ...
    pub fn subscribe_permission_requests_for_agent<F>(&self, agent_name: String, callback: F) -> SubscriptionId
    where
        F: Fn(&PermissionRequestEvent) + Send + Sync + 'static,
    {
        self.subscribe_with_filter(
    ...
    pub fn subscribe_workspace_updates_for_workspace<F>(&self, workspace_id: String, callback: F) -> SubscriptionId
    where
        F: Fn(&WorkspaceUpdateEvent) + Send + Sync + 'static,
    {
        self.subscribe_with_filter(
        ...
        F: Fn(&Config) + Send + Sync + 'static,
    {
        self.subscribe_with_filter(
            ...
            |event| matches!(event, AppEvent::AgentConfig(AgentConfigEvent::ConfigReloaded { .. })),
        )
    }

    pub fn subscribe_agent_config_for_agent<F>(&self, agent_name: String, callback: F) -> SubscriptionId
    where
        F: Fn(&AgentConfigEvent) + Send + Sync + 'static,
    {
        self.subscribe_with_filter(
...
mod tests {
    ...
    fn test_publish_subscribe() {
        ...
        hub.subscribe_code_selections(move |event| {
            received_clone.lock().unwrap().push(event.selection.file_path.clone());
        });
